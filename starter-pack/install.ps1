...
if ((Test-Path (Join-Path $resolved $instructionFile)) -or (Test-Path (Join-Path $resolved "VDAI_AI_STARTER_VERIFICATION.md")) -or (($targetKind -eq ".claude") -and (Test-Path (Join-Path $resolved "VDAI_AI_STARTER_USAGE.md"))) -or ($skills | Where-Object { Test-Path (Join-Path $resolved "skills\$_") })) {
...
Write-Host "VDAI AI Starter · Dmitrii Pro installed. Restart the agent and run both turns from VDAI_AI_STARTER_VERIFICATION.md. Claude Code users: review VDAI_AI_STARTER_USAGE.md before merging statusLine settings."
